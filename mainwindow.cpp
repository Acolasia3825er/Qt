...
#include <QJsonObject>
#include <QJsonArray>
#include "weather_citycode.h"
#include <QPainter>

#define INCREMENT  3   // 温度每升高或每降低一度，y轴坐标的增量
    ...
    , ui(new Ui::MainWindow)
{
    ...
    mTypeMap.insert("中雨",":/res/type/BaoXue.png");
    ...
    //请求天气数据，"101270101"是成都天气编码
//    getWeatherInfo("101270101");
    getWeatherInfo("成都");  //成都
...
void MainWindow::contextMenuEvent(QContextMenuEvent *event)    //弹出右键菜单
{
...
void MainWindow::parseJson(QByteArray &byteArray)
{
    ...
    QJsonObject objYestoday = objData.value("yesterday").toObject();


    mDay[0].week = objYestoday.value("week").toString();
    mDay[0].date = objYestoday.value("ymd").toString();
    mDay[0].type = objYestoday.value("type").toString();

    QString s;
    s = objYestoday.value("high").toString().split(" ").at(1); //18°
    s = s.left(s.length()-1); //18
    mDay[0].high = s.toInt();


    s = objYestoday.value("low").toString().split(" ").at(1); //18°
    s = s.left(s.length() - 1);
    mDay[0].low = s.toInt();


    //风向风力
    mDay[0].fx = objYestoday.value("fx").toString();
    mDay[0].fl = objYestoday.value("fl").toString();

    //污染指数
    mDay[0].aqi = objYestoday.value("aqi").toDouble();

    //3.解析forecast中5天的数据
    QJsonArray forcastArray = objData.value("forecast").toArray();

    for(int i = 0;i < 5; i++){
        QJsonObject objForcast = forcastArray[i].toObject();
...
void MainWindow::painHighCurve()
{
    ...
    pen.setWidth(1);        //画笔的宽度
    pen.setColor(QColor(255,170,0));   //画笔的颜色
...
void MainWindow::painLowCurve()
{
    ...
    QPen pen = painter.pen();
    pen.setWidth(1);        //画笔的宽度
    pen.setColor(QColor(0,255,255));   //画笔的颜色
